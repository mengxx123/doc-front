(function (factory) {
    ...
    // 验证是否引用jquery
    if (!$ || !$.fn || !$.fn.jquery) {
        alert('在引用eEditor.js之前，先引用jQuery，否则无法使用 eEditor');
        return;
    }
    ...
    (function (window, $) {
        if (window.eEditor) {
            // 重复引用
            alert('一个页面不能重复引用 eEditor.js 或 eEditor.min.js ！！！');
            return;
        }
        ...
        // 用户在引用 eEditor.js 之后，还可以通过 E.plugin() 注入自定义函数，
    ...
    _e(function (E, $) {
        ...
        E.fn._queryCommandValue = function (commandName) {
            var result = '';
            try {
        ...
        // IE8 直接执行偶尔会报错，因此直接用 try catch 封装一下
        E.fn._queryCommandState = function (commandName) {
            var result = false;
            try {
...
;(function () {
    ...
    E.addCommand = function (cmd) {
        E.cmds[cmd.name] = {
            ...
            systemCmd: cmd.systemCmd
        };
    };

    E.fn.execCommand = function (cmd, value) {
        if (E.cmds[cmd]) {
            E.cmds[cmd].handler.call(this, this);
        }
    };
